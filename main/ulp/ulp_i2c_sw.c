...
// Source: https://github.com/PascalPolygon/stm32_bitbang_i2c
...
_Bool I2C_read_pin(gpio_num_t gpioPinToRead) {
    ulp_riscv_gpio_input_enable(gpioPinToRead);
    _Bool b = ulp_riscv_gpio_get_level(gpioPinToRead);
    ulp_riscv_gpio_input_disable(gpioPinToRead);
    return b;
}

uint8_t I2C_read_SDA(void)
{
    if (I2C_read_pin(ULP_I2C_SW_SDA_GPIO) == true)
        return 1;
    else
        return 0;
    return 0;
